...
build_all()
{
rsync -av . ~/Documents/backup/libbitnew/libbitcoin-node
if [ ! -d "$BUILD_DIR/libbitcoin-node" ]; then
    ...
    build_from_github PublicStartup libbitcoin-blockchain master $PARALLEL ${BITCOIN_BLOCKCHAIN_OPTIONS[@]} "$@"
    build_from_github PublicStartup libbitcoin-network master $PARALLEL ${BITCOIN_NETWORK_OPTIONS[@]} "$@"
    build_from_travis PublicStartup libbitcoin-node master $PARALLEL ${BITCOIN_NODE_OPTIONS[@]} "$@"
else
